...
public class InventoryDAO {
    ...
    public List<StockModel> getAllStock(String keyword, String brand, String status) {
        ...
        StringBuilder sql = new StringBuilder(
          ...
          + "p.Name AS product_name, p.Image AS p_image, p.Status AS product_status, "
          + "p.Latest_cost, b.Name AS brand_name, c.Name AS category_name, "
    ...
    public boolean stockIn(int variantId, int qty, String note, Integer createdBy) {
        String checkSql = "SELECT COUNT(*) FROM Inventory WHERE VariantID = ?";
        String insertSql = "INSERT INTO Inventory(VariantID, Amount, Status, Updated_at) VALUES(?, ?, 'ACTIVE', CURRENT_TIMESTAMP)";
        String updateSql = "UPDATE Inventory SET Amount = Amount + ?, Updated_at = CURRENT_TIMESTAMP WHERE VariantID = ?";
        try (Connection conn = DBContext.getConnection()) {
    ...
    public boolean stockAdjust(int variantId, int newStock, String note, Integer createdBy) {
        String insertSql = "INSERT INTO Inventory(VariantID, Amount, Status, Updated_at) VALUES(?, ?, 'ACTIVE', CURRENT_TIMESTAMP)";
        String updateSql = "UPDATE Inventory SET Amount = ?, Updated_at = CURRENT_TIMESTAMP WHERE VariantID = ?";
        try (Connection conn = DBContext.getConnection()) {
            conn.setAutoCommit(false);
            try {
                try (PreparedStatement ps = conn.prepareStatement(
                        "INSERT INTO Inventory(VariantID, Amount, Status, Updated_at) VALUES(?, ?, 'ACTIVE', CURRENT_TIMESTAMP) "
